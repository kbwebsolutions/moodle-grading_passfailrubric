...
/**
 * File contains definition of class MoodleQuickForm_passfailrubriceditor
 *
...
/**
 * Form element for handling passfailrubric editor
 *
 * The passfailrubric editor is defined as a separate form element. This allows us to render
 * criteria, levels and buttons using the passfailrubric's own renderer. Also, the required
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class MoodleQuickForm_passfailrubriceditor extends HTML_QuickForm_input {
    ...
    /** @var bool Message to display in front of the editor (that there exist grades on this passfailrubric being edited) */
    ...
    /**
     * Specifies that confirmation about re-grading needs to be added to this passfailrubric editor.
     * $changelevel is saved in $this->regradeconfirmation and retrieved in toHtml()
     *
     * @see gradingform_passfailrubric_controller::update_or_check_passfailrubric()
    ...
    public function toHtml() {
        global $PAGE;
        $html = $this->_getTabs();
        $renderer = $PAGE->get_renderer('gradingform_passfailrubric');
        $data = $this->prepare_data(null, $this->wasvalidated);
        if (!$this->_flagFrozen) {
            $mode = gradingform_passfailrubric_controller::DISPLAY_EDIT_FULL;
            $module = array(
                'name' => 'gradingform_passfailrubriceditor',
                'fullpath' => '/grade/grading/form/passfailrubric/js/passfailrubriceditor.js',
                'requires' => array('base', 'dom', 'event', 'event-touch', 'escape'),
                'strings' => array(
                    array('confirmdeletecriterion', 'gradingform_passfailrubric'),
                    array('confirmdeletelevel', 'gradingform_passfailrubric'),
                    array('criterionempty', 'gradingform_passfailrubric'),
                    array('levelempty', 'gradingform_passfailrubric')
                )
            );
            $PAGE->requires->js_init_call(
                    'M.gradingform_passfailrubriceditor.init',
                    array(
        ...
        $html .= $renderer->display_passfailrubric($data['criteria'], $data['options'], $mode, $this->getName());
        return $html;
    }
     ...
     * - if options not passed (i.e. we create a new passfailrubric) fills the options array with the default values
    ...
    protected function prepare_data($value = null, $withvalidation = false) {
        ...
        $totalscore = 0;
        $errors = array();
        $return = array('criteria' => array(), 'options' => gradingform_passfailrubric_controller::get_default_options());
        ...
        foreach ($value['criteria'] as $id => $criterion) {
            ...
            /* This is legacy stuff as the levels can be hard coded */
            $levels = [
                'NEWID0' => ['definition' => get_string('refer','gradingform_passfailrubric')],
                'NEWID1' => ['definition' => get_string('fail','gradingform_passfailrubric')],
                'NEWID2' => ['definition' => get_string('pass','gradingform_passfailrubric')]
            ];
            //mavg
        ...
        if ($withvalidation) {
            if (count($errors)) {
                ...
                foreach ($errors as $error => $v) {
                    $rv[] = get_string($error, 'gradingform_passfailrubric');
                }
    ...
    /**
     * Validates that passfailrubric has at least one criterion, at least two levels within one criterion,
